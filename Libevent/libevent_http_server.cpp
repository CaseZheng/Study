...
void callback_home(struct evhttp_request *request, void *arg)
{
    const struct evhttp_uri * uri = evhttp_request_get_evhttp_uri(request);
    char url[8192] = {0};
    evhttp_uri_join(const_cast<struct evhttp_uri*>(uri), url, 8192);
    ...
    evbuffer_add_printf(evbuf, "%s", "home hello libevent http");
    evhttp_send_reply(request, HTTP_OK, "OK", evbuf);
    evbuffer_free(evbuf);
}
...
int main()
{
    ...
    evhttp_set_cb(http, "/home/", callback_home, NULL);
